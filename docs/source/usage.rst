...
#.  The BLACS checkbox: If ticked prior to clicking the engage button, shot files are put
    into the runmanager shot queue once the hardware instructions have been generated by
    labscript, for BLACS to collect when it next asks for a shot. It decides only where
    newly engaged shots go: it is not *Pause queue*, which withholds shots already in the
    queue, and it is not BLACS's own *Request shots*, which is what lets BLACS run
    anything at all. The indicator beside it shows what BLACS is doing and stays live
    whether the checkbox is ticked or not. This control was previously labelled ‘run
    shots’; only the label changed, and its remote methods are still named
    ``get_run_shots``/``set_run_shots``.
...
#.  The BLACS hostname: Runmanager no longer carries a field for this. The hostname and
    port of the PC running BLACS come from the ``servers`` and ``ports`` sections of the
    laboratory PC configuration file (see labconfig in the glossary), and are where
    runmanager polls for BLACS's status. It is expected that BLACS is running on the
    specified PC, and that network access (including firewalls and other network access
    controls) is configured appropriately. Shot files are not pushed to that host — BLACS
    asks runmanager for them.
...
The queue is on runmanager's Queue tab, along with the *Compile mode* and *When
queue is empty* settings and the *Pause queue* checkbox. Each queued shot is
...
goes on running its own local override shot meanwhile. It is saved and restored
with the queue configuration, and a configuration written before the control
existed opens with the queue running. The full contract between the two
applications — ownership, exchange ordering, stable identifiers, retry rules
and the status query — is written up in BLACS's *Shot Management* page.
...
work. A runmanager with the same *When queue is empty* setting makes another as
soon as it is asked.
